...
@section("wrapper")
    <div>
        <div>
            <x-breadcrumbs class="mt-2"  :links="[['url'=>route('job-title.index'),'label'=>'Должности'],['label'=>'Добавить должность']]"></x-breadcrumbs>
            <div class="bg-white border border-4 rounded-lg shadow relative my-10">
                ...
                <div class="p-6 space-y-6">
                    ...
                    @endif
                    <form id="form" action="{{ route('job-title.store') }}" method="POST" autocomplete="off">
                        @csrf
                        @method('post')
                            ...
                            <div class="col-span-full">
                                <label for="inputType" class="text-sm font-medium text-gray-900 block mb-2">Категория</label>
                                    <select id="inputType"
                                        ...
                                        <option value=1 {{ old('roleType') == 1 ? 'selected' : '' }}> Преподователь </option>
                                        <option value=2 {{ old('roleType') == 2 ? 'selected' : '' }}> Сотрудник </option>
                                        <option value=-1 {{ old('roleType') == -1 ? 'selected' : '' }}> Другое </option>
                                    </select>
                            </div>
                            <div class="col-span-full">
