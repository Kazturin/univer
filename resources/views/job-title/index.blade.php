...
@section("wrapper")
    <div>
        <div class="mb-3">
            <x-breadcrumbs class="mt-2"  :links="[['label'=>'Должности']]"></x-breadcrumbs>
            ...
            <div class="flex justify-between">
                ...
                <div>
                    <a href="{{route('job-title.create')}}" class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        {{ __('button.add') }}
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-lg border-t mb-5">
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap shadow-lg">
                        ...
                        >
                            <th class="px-4 py-3">Наименование(kz)</th>
                            <th class="px-4 py-3">Наименование(ru)</th>
                            <th class="px-4 py-3">Наименование(en)</th>
                        ...
                        @foreach($jobTitles as $jobTitle)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">{{ $jobTitle->NameKZ }}</td>
                                <td class="px-4 py-3"> {{ $jobTitle->NameRU }} </td>
                                <td class="px-4 py-3">{{ $jobTitle->NameEN }}</td>
                                <td class="px-4 py-3">{{ $categories[$jobTitle->roleType] }}</td>
                                <td class="flex items-center space-x-4 text-sm">
                                    <a href="{{ route('job-title.edit', $jobTitle->ID) }}"
                                    ...
                                    <form action="{{ route('job-title.destroy',$jobTitle->ID) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button
