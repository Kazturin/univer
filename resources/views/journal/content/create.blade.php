...
@section("wrapper")
            ...
            <x-breadcrumbs class="mt-2"  :links="[['url'=>route('content.index'),'label'=>'Учебный контент дисциплины'],['label'=>'Добавить контент']]"></x-breadcrumbs>

            <div class="bg-white border border-4 rounded-lg shadow relative my-6">
                <div class="flex items-start justify-between p-5 border-b rounded-t">
                    <h3 class="text-xl font-semibold">
                        Добавить учебный контент
                    </h3>
                ...
                <div class="p-6 space-y-6">
                    ...
                    @endif
                    <form id="form" action="{{ route('content.store') }}" method="POST" autocomplete="off">
                        @csrf
                        @method('post')
                        <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-full">
                                <label for="inputName" class="text-sm font-medium text-gray-900 block mb-2">Наименование учебного контента</label>
                                <input type="text"
                                       id="inputName"
                                       name="contentname"
                                       value="{{ old('contentname') }}"
                            ...
                            <div class="col-span-full">
                                ...
                                <select id="inputLang"
                                    ...
                                    <option value=2 {{ old('lang') == 2 ? 'selected' : '' }}> Казахский </option>
                                    <option value=1 {{ old('lang') == 1 ? 'selected' : '' }}> Русский </option>
                                    <option value=3 {{ old('lang') == 3 ? 'selected' : '' }}> Английский </option>
                                </select>
                            </div>
                        </div>
                    </form>
