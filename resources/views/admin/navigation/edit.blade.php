...
@section("wrapper")
    ...
    <div class="page-wrapper">
        <div class="page-content">
            <!--breadcrumb-->
            <x-breadcrumbs class="mt-2"  :links="[['url'=>route('navigation.index'),'label'=>'Навигация'],['label'=>'Редактировать навигацию']]"></x-breadcrumbs>
            <!--end breadcrumb-->
            ...
            <div class="p-6 space-y-6">
                ...
                @endif
                <form id="form" action="{{ route('navigation.update', $navigation->id) }}" method="POST" autocomplete="off">
                    @csrf
                    @method('put')
                        ...
                        <div class="col-span-full">
                            ...
                            <select id="inputParent" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" name="parent_id">
                                ...
                                @foreach($navigations as $nav)
                                    <option value="{{$nav->id}}" {{ $navigation->parent_id===$nav->id ? 'selected' : ''  }}>{{$nav->name_kz}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-span-full">
            ...
            <div class="p-6 border-t border-gray-200 rounded-b">
                <button onclick="submitForm()" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="submit">Изменит</button>
            </div>
        </div>
    </div>
    </div>
