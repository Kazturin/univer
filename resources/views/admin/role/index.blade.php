...
@section("wrapper")
    <div>
        <div class="mb-3">
            <x-breadcrumbs class="mt-2"  :links="[['label'=>'Роли']]"></x-breadcrumbs>
            ...
            <div class="flex justify-between">
                ...
                <div>
                    <a href="{{route('role.create')}}" class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                        {{ __('button.add') }}
                    ...
                    <table class="w-full whitespace-no-wrap shadow-lg">
                        ...
                        @foreach($roles as $role)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">{{ $role->RoleNameKZ }}</td>
                                <td class="px-4 py-3"> {{ $role->RoleNameRu }} </td>
                                <td class="flex items-center space-x-4 text-sm">
                                    ...
                                    <form action="{{ route('role.destroy',$role->RoleID) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button
